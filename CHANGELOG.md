...
- **GrumPHP** (`phpro/grumphp` ^2.20) : `grumphp.yml` à la racine. Hooks pre-commit (composer + phpcsfixer + twigcsfixer + lint twig + yamllint) et pre-push (phpstan + securitychecker_symfony). Hook git auto-installé via `composer install`. Contournement ponctuel via `git commit --no-verify`
