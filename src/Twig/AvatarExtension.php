...
final class AvatarExtension extends AbstractExtension
{
    public function __construct(
        private readonly UserAvatarResolver $resolver,
    ) {
    }
    ...
    public function renderAvatar(User $user, int $size = 64, string $class = ''): Markup
    {
        ...
        $html = $render->isInline()
            ? self::wrapSvg($render, $cssClass)
            : self::wrapImg($render, $cssClass);

        return new Markup($html, 'UTF-8');
    }

    private static function wrapImg(AvatarRender $render, string $cssClass): string
    {
    ...
    private static function wrapSvg(AvatarRender $render, string $cssClass): string
    {
