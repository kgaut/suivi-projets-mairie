 ...
 * initiales.
 */
final readonly class AuthentikAvatarFetcher
{
    ...
    public function fetchIfNeeded(User $user, ?string $pictureUrl): void
    {
        ...
        if (!$this->needsRefresh($user, $pictureUrl)) {
            return;
        }
        ...
        try {
            ...
            $this->logger->warning(
                ...
                ['user' => $user->getAuthentikId(), 'reason' => $throwable->getMessage()],
            );
    ...
    private function needsRefresh(User $user, string $newUrl): bool
    {
        ...
        try {
            ...
            foreach ($this->httpClient->stream($response) as $chunk) {
                ...
                if (\strlen($bytes) > self::MAX_SIZE_BYTES) {
                    throw new RuntimeException(\sprintf('Avatar exceeds max size %d bytes.', self::MAX_SIZE_BYTES));
                }
            }
        } catch (TransportException|ExceptionInterface $e) {
            throw new RuntimeException('HTTP error: ' . $e->getMessage(), $e->getCode(), previous: $e);
        }

        return $this->resizeToWebP($bytes);
    }

    private function resizeToWebP(string $sourceBytes): string
    {
