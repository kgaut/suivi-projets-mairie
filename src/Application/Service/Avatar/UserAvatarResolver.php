...
final class UserAvatarResolver
{
    ...
    public function resolve(User $user, int $size = 64): AvatarRender
    {
        ...
        return AvatarRender::fromSvg(self::initialsSvg($user, $size), $alt, $size);
    }
     ...
     */
    private static function initialsSvg(User $user, int $size): string
    {
        $initials = self::initialsOf($user->getDisplayName());
        $bg = self::colorFor($user->getAuthentikId());
    ...
    private static function initialsOf(string $displayName): string
    {
    ...
    private static function colorFor(string $authentikId): string
    {
